...
| Hemisphere+Directional+Point(LED) | RectAreaLight(네온) |
|---|---|
| ![](screenshots/25-light-baseline.png) | ![](screenshots/30-light-all-off-neon-left.png) |

LED가 켜진 채로는 네온의 기여가 노출에 묻혀 한 컷에 4종을 동시에 보여주기 어렵다 — 자세한 분해는 3-11 참고.
...
| 환경맵 | 스카이박스(근거리 사진 기반)와 3D 바닥 지오메트리가 시각적으로 매칭되지 않음. `scene.environment`는 설정하지 않아 IBL(조명)로는 안 쓰이고 `scene.background`(배경 그림)로만 사용됨 | 큐브맵은 무한히 먼 배경으로 렌더되는데, 실제 바닥/벽은 유한한 8×8 박스라 지면 연결부에서 이질감이 남음 | 야외에서 둘러보면 "배경 따로, 바닥 따로"인 느낌이 남음. 야외에서 보이는 빛은 스카이박스가 아니라 범위 제한 없는 `DirectionalLight`/`HemisphereLight`/GI가 실내와 똑같이 비치는 것. `clampToStore` 경계로 너무 가까이 가는 것만 막아 직접적인 노출은 줄임 |
